...
## In the section we have pre-built an Amazon Bedrock Agent with five action groups for you. You will a) review the agent configurations b) add knowledge bases c) configure advance prompts 
 ...
 1. On the left side bar of Bedrock console, click **Agents**. Then click **network_fault_troubleshooting_agent**. Then click **Edit in Agent Builder**. Review the following: 
   - Under **Agent details** review **Agent name:** `network_fault_troubleshooting_agent` 
   - Under **Select model:** `Claude 3 Sonnet v1` (follow the red arrows in below pictures). 

![agent](/static/module2images/agent.png)
![agent](/static/module2images/nfta81.png)
![agent](/static/module2images/nfta82.png)
![agent](/static/module2images/nfta130.png)

2. Scroll down to **Action Groups**. In this agent, five action groups are pre-built. You should review them to understand what is the role of each action group (follow the red box in below picture).

![add kb](/static/module2images/f2.png) 

3. Now let's review the first action group configurations - billstatus
      - Click **billstatus**
...
4. Now let's review the second action group configurations- customer_authentication
...
5. Now let's review the third action group configurations- modem_status
      - Click **modem_status**
...
6. Now let's review the fourth action group configurations- node_status
      - Click **node_status**
...
7. Now let's review the fifth and the last action group configurations - speedstatus
      - Click **speedstatus**
...
8. Congratulations! You have reviewed all five action groups. 
9. Under **Instructions for the Agent** review the following instruction (follow the red arrow in below picture).
...
8. Troubleshooting Logic:
    a) If speedstatus agent reports slow speed, check bill payment status. Remember non-payment results in slow network speed and it will never make the network down
...
![add kb](/static/module2images/nfta65.png) 

10. Under **Additional settings**, go to **User input**, then click **Enabled** (follow the red arrow in below picture).

![add kb](/static/module2images/usinput.png)

11. If all is good then scroll all the way up and click **Save** (follow the red arrow in below picture).

![add kb](/static/module2images/nfta3.png)

12. Now we will add two knowledge bases created previously. 
13. Let's add the first knowledge base. Scroll down to **Knowledge bases** and click **Add**. Select **modemtroubleshoot-kb**, under **Knowledge base instructions for Agent** put `this knowledge base is used for troubleshooting customer modem if modem is not working. Provide provide step by step troubleshooting` (follow the red in arrows below pictures).

![add kb](/static/module2images/kb1add.png)
![add kb](/static/module2images/kb1details.png) 

14. Let's add the second and the last knowledge base. Scroll down to **Knowledge bases** and click **Add**. Select **billpayment-kb**, under **Knowledge base instructions for Agent** put `This knowledge base contains broadband bill payment for IOS, Android, MacOS and Windows. Ask the customer which device they use then provide step by step help to the users.` (follow the red arrows below pictures).

![add kb](/static/module2images/kb1add.png)
![add kb](/static/module2images/kb2details.png) 

15. Congratulations! You have added 2 knowledge bases. 

16. Scroll all the way to the bottom to **Advanced Prompts**. Click **Edit**
17. Under **Advanced Prompts** (follow the red arrows in below pictures).
...
18. In the agent console, scroll up and **Save and Exit**. After **Save and Exit**, scroll-up to read the message in the green banner. It is asking to prepare the agent **Agent: network_fault_troubleshooting_agent was successfully updated. Prepare the agent to keep its detail up to date**. 
...
19. Click **Create Alias**. Set following values: 
    - **Alias name:** `networkfta` 
    - **description:** `agent alias`
- Click **Create Alias** at the bottom (follow the red arrows and red boxes in below pictures). 

![add kb](/static/module2images/ca.png)
![add kb](/static/module2images/ca2.png)
